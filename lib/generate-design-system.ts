...
const MIRROR: Record<Step, Step> = {
  xxd: "xxl", xd: "xl", d: "l", base: "base", l: "d", xl: "xd", xxl: "xxd",
};
...
const normalize = (cfg: SemanticConfig) =>
  typeof cfg === "string" ? { name: cfg, invert: false, dark: {} as Record<string, string> }
...
if (BRICKS) {
  ...
  writeFileSync(colorOutputPath, "/* Colours provided by Bricks (palette import generates tokens + utilities). */\n");
  ...
  const utilityFor = (token: string) =>
    UTILITIES
      .map((cls) => `.${cls}-${token} { ${UTILITY_PROPS[cls]}: var(--color-${token}); }`)
      .join("\n") + "\n";

  const allTokens: string[] = [];
  for (const [name, steps] of Object.entries(named))
    for (const step of STEP_ORDER) if (steps[step] != null) allTokens.push(`${name}${suffix(step)}`);
  for (const [role, cfg] of Object.entries(semantic)) {
    const { name } = normalize(cfg);
    for (const step of STEP_ORDER) if (named[name]?.[step] != null) allTokens.push(`${role}${suffix(step)}`);
  }
...
const stateRules = (
  ...
  for (const [state, spec] of Object.entries(states)) {
    ...
    if (spec.css && typeof spec.css === "object") {
      for (const [k, v] of Object.entries(spec.css as Record<string, string>)) body.push(`${k}: ${v};`);
    }
    if (!body.length) continue;

    const pseudo = state === "hover" ? ":hover" : state === "active" ? ":active"
                 : state === "focus-visible" ? ":focus-visible" : `:${state}`;
...
for (const [pname, p] of Object.entries(patterns ?? {})) {
  ...
  const defaultRole = p.default_role
    ? (semantic[p.default_role.replace(/^.*$/, p.default_role)] ? p.default_role : p.default_role)
    : null;
  ...
  const fills = pname === "button" || pname === "badge" ||
    base["background-color"] != null || base["background"] != null;
  ...
  const hasStates = Object.keys(p.states).length > 0;
  if (hasStates) {
  ...
  pat += stateRules(defaultSel, defaultRole, p.states, colorProp);
  ...
  for (const role of p.roles) {
    ...
    pat += stateRules(variantSel, role, p.states, colorProp);
  }
  pat += "\n";
}
...
writeFileSync(join(distPath, "bricks-colors-semantic.json"), JSON.stringify(semanticPalette, null, 2));
console.log(
