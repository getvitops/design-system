...
| Key          | Controls                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `colors`     | `palette` hues (seeded or fixed-`tones` ramps → 11-step OKLCH scales `50…950` on a shared lightness ladder), `roles` mapping semantic roles (`neutral`, `surface`, `ui-primary`, `brand-primary`, `success`, `danger`, …) onto hues, and which `utilities` emit (`bg`/`text`/`icon`/`border`). Tokens are `--color-<target>-<role>[-<variant>]` and the class name is that minus `--color-`. A role is `kind: "surface"` (has a bare `bg-<role>`) or chromatic (tints + solids, **no** bare `bg-<role>`). The dark-mode flip derives from this. |
...
| `patterns`   | Component patterns (`items`: cta/btn/link/badge/card/… with `base`, `states`, `roles`), plus `defaults`, `radii`, `groups`, `z`. Drives `patterns.css`. `.cta` = persuasion (filled, opt-in, works on `<a>`); bare `<button>`/`.btn` = affordance.                                                                                                                                                                                                                                                                                              |
...
- **Which class does X?** → search `docs/css/classes.md` (rules + token lists). Apply the
  rule to a listed token (e.g. role `bg-<role>[-<variant>]` → `bg-surface`,
  `bg-danger-muted`, `bg-danger-solid`; raw `bg-<hue>-<step>` → `bg-navy-100`;
  `split-<a>-<b>`, breakpoint-prefixable as `md-split-1-2`).
