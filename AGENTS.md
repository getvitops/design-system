...
- **`@getvitops/cli`** — `vitops generate|init|validate|favicon|agents` (bin `vitops`), a thin
  wrapper over the generator + utils. `agents` writes a managed design-system block into a consumer's
  `AGENTS.md` (+ emits the docs bundle) so AI agents can discover the system.
