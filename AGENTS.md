...
| `apps/docs` (Astro + Starlight)     | public docs site                        | hybrid — see below     |

`apps/docs` is private and **isolated from the release DAG** like `apps/portal` (not in `build`, `build:packages` or `deploy`). Its _Reference_ section is not written by hand: `apps/docs/scripts/sync-reference.mjs` calls the generator's `generateDocs()` and emits Starlight pages into `src/content/docs/reference/` (gitignored), so the site can't describe output the toolchain doesn't produce. The script flattens the OKF tree (`concepts/patterns.md` → `concepts-patterns`), drops the frontmatter-less `index.md` listings (Starlight's sidebar replaces them), and rewrites the bundle's relative `.md` cross-links to Starlight slugs. Guides and package pages under `guides/` and `packages/` **are** hand-written. Tasks: `docs:sync` (dependsOn `build:generator`), `docs:dev`, `docs:build`, `docs:preview`.
