...
export function resolveIcon(name: string, prefix: string): string {
  ...
  if (!mapKey) throw new Error(`Unknown icon prefix "${prefix}". Valid: ${Object.keys(prefixToMapKey).join(', ')}`);
  const map = iconMap[mapKey];
  const resolved = map[name as keyof typeof map];
  if (!resolved) throw new Error(`Icon "${name}" not found in set "${prefix}" (map key: "${mapKey}")`);
...
export function resolveBrandIcon(name: string, prefix: string): string {
  ...
  const map = iconMap[mapKey];
  const resolved = map[name as keyof typeof map];
  if (!resolved) throw new Error(`Brand icon "${name}" not found in set "${prefix}" (map key: "${mapKey}")`);
  return `${prefix}:${resolved}`;
}
 ...
 */
export function generateIconInclude(
  iconsConfig?: {
    ui?: string;
    brand?: string;
    semantic?: string[];
  ...
  for (const name of iconsConfig?.semantic ?? []) {
    ...
    if (uiMapKey) {
      const r = (iconMap[uiMapKey] as Record<string, string>)[name];
      if (r) addToInclude(uiPrefix, [r]);
    }
    const brandMapKey = prefixToMapKey[brandPrefix] as IconSet | undefined;
    if (brandMapKey) {
      const r = (iconMap[brandMapKey] as Record<string, string>)[name];
      if (r) addToInclude(brandPrefix, [r]);
    }
  }
