 ...
 */
export function gamutMapOklchToHex(l: number, c: number, h: number, space: 'srgb' | 'p3' = 'srgb'): string {
  const col: PlainColorObject = { space: OKLCH_Space, coords: [l, c, h], alpha: 1 };
...
export const COLOR_GROUPS = ["neutral", "brand", "primary", "accent", "complement", "success", "warning", "error", "info"] as const;
...
export const HARMONY_TYPES = ['single', 'analogous', 'triad', 'complementary', 'split-complementary', 'square', 'compound'] as const;
...
export function parseOklch(oklchString: string): OklchColor {
    ...
    const l = parseFloat(parts[1]) / (parts[1].includes('%') ? 100 : 1);
 ...
 */
function transposeLightnessScale(parsedAnchors: Record<StepName, OklchColor>, sortedAnchorSteps: StepName[]): Record<StepName, number> {
    const transposedScale: Record<number, number> = {};

    if (sortedAnchorSteps.length === 1) {
        const anchorStep = sortedAnchorSteps[0];
        const anchorL = parsedAnchors[anchorStep].l;
    ...
    for (const step of STEP_NAMES) {
        if (parsedAnchors[step]) {
            transposedScale[step] = parsedAnchors[step].l;
            continue;
        }
        ...
        for (let i = sortedAnchorSteps.length - 1; i >= 0; i--) {
            if (sortedAnchorSteps[i] < step) {
                lowerStep = sortedAnchorSteps[i];
                break;
            }
        }
        ...
        if (lowerStep !== null && upperStep !== null) {
            const lowerL = parsedAnchors[lowerStep].l;
            const upperL = parsedAnchors[upperStep].l;
 ...
 */
export function generateOklchPalette(input: string | string[] | Record<string, string>): Record<StepName, string> {
    ...
    if (typeof input === 'string') {
        ...
        throw new Error('Input must be a color string, array of colors, or object with step assignments');
    }

    const parsedAnchors: Record<number, OklchColor> = {};

    for (const key in anchors) {
        parsedAnchors[parseInt(key)] = parseOklch(anchors[key]);
    }
    ...
    const lightnessScale = transposeLightnessScale(parsedAnchors as Record<StepName, OklchColor>, sortedAnchorSteps);

    const palette: Record<number, string> = {};

    for (const step of STEP_NAMES) {
        if (parsedAnchors[step]) {
            palette[step] = anchors[step];
            continue;
        }
        ...
        if (startStep !== null && endStep !== null) {
            const startColor = parsedAnchors[startStep];
            const endColor = parsedAnchors[endStep];
            ...
            c_int = parsedAnchors[startStep].c;
            h_int = parsedAnchors[startStep].h;
        } else if (endStep !== null) {
            c_int = parsedAnchors[endStep].c;
            h_int = parsedAnchors[endStep].h;
        } else {
            c_int = 0.1;
            h_int = 0;
        }

        const l_int = lightnessScale[step];

        const dampeningFactor =
            (l_int < 0.25) ? 0.3 + (l_int / 0.25) * 0.7 :
            (l_int > 0.75) ? 0.3 + ((1 - l_int) / 0.25) * 0.7 :
            1.0;
